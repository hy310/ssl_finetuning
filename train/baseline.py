...
class PronunciationScoringModel(nn.Module):
    def __init__(self, model_name, model_save_path):
        ...
        # Linear layer to predict 4 pronunciation scores [1024,4]
        self.score_predictor = nn.Linear(config.hidden_size, 4)
        
    def forward(self, input_values, labels=None):
        outputs = self.model(input_values=input_values)
        last_hidden_state = outputs.last_hidden_state
        pooled_output = last_hidden_state.mean(dim=1)  # [batch, hidden states]
        scores = self.score_predictor(pooled_output)
        loss = None
        if labels is not None:
            loss_fct = nn.MSELoss() # MSE Loss (CTC X)
...
def prepare_arguments():
    ...
    parser.add_argument("--save_dir", type=str, default="/data2/haeyoung/0303/general/model_saved", help="Directory to save the trained model.")
